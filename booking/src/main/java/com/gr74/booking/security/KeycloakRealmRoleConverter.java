 ...
 * {@code "realm_access": {"roles": ["USER", "ADMIN"]}}. With the default converter every
 * {@code hasRole('ADMIN')} silently evaluates to {@code false}: no exception, no log line, just a
 * 403 on every admin call while user calls keep working — the single most common Keycloak + Spring
 * misconfiguration, and the reason this converter gets its own unit test
 * ({@code KeycloakRealmRoleConverterTest}) proving a token with
 * {@code realm_access.roles=["ADMIN"]} actually yields {@code ROLE_ADMIN}.
 *
