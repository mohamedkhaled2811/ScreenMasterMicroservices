...
- [ ] **2.2 "My bookings" — way A (API composition):** Booking calls Catalog over HTTP (`RestClient`) and merges titles in memory. *(Deferred to its own branch — the Catalog data it needs now exists via 2.1a.)* *(Concepts: [spring-web-annotations.md](concepts/spring-web-annotations.md), [database-per-service.md](concepts/database-per-service.md))*
