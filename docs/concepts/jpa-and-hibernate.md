...
- **Detached entities are read-only** — a *write* transaction must re-load what it writes. We hit this on the payment "Pay Again" path (`PaymentWriter.insertPendingAttempt`): the checkout path commits each write in its own `REQUIRES_NEW` transaction (a DB transaction must not span the external gateway call), so the `Payment` returned by one boundary is **detached** by the next. A freshly *inserted* payment's `attempts` is a plain `ArrayList`, so mutating it happened to work; a *loaded* one's is an uninitialized lazy collection bound to the dead session → `LazyInitializationException` on `add`. The first "Pay" masked the bug; the retry exposed it. Fix: the write transaction loads its own managed copy (`payments.findById(...)`) before mutating — the same rule `recordSession` already followed. Regression test: `PaymentTransactionBoundaryTest#secondAttemptOnALoadedPaymentOpensANewSession`. Interview lens: *"when is a REQUIRES_NEW boundary also an entity-detachment boundary, and what breaks when you cross it?"* — real transaction boundaries (unlike inert ones from self-invocation) detach every entity they return.
