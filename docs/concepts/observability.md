...
**The demo.** One booking: `grep` all five container logs by one trace id → one story, in order; open the Zipkin waterfall for that same id → the synchronous session trace, the webhook trace tagged with `paymentId`, and the confirm→email chain hanging off the webhook trace.
