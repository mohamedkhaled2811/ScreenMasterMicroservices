 ...
 * unit test proving a token with {@code realm_access.roles=["ADMIN"]} actually yields
 * {@code ROLE_ADMIN}. The gateway enforces no roles <em>today</em> (coarse edge split only), but
 * the converter is wired from day one so the first role-gated route works instead of silently
 * denying.
 *
