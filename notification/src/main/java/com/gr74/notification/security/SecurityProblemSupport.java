 ...
 * {@code ProblemDetail} shape the other services produce — with the same machine-readable
 * {@code code} property.
 *
 * <p><b>Why the filter chain needs its own renderer:</b> security rejections happen in the servlet
 * filter chain <em>before</em> any controller runs (no token, expired token, valid token without
 * the role), so without this class they fall back to the container's default: an empty 401/403
 * with no body. Notification has no business controllers (it is a pure event consumer), which
 * makes this class its <em>entire</em> HTTP error surface today — and the reason
 * {@code NotificationErrorCode} carries {@code NOTIFICATION_UNAUTHORIZED}/{@code
 * NOTIFICATION_FORBIDDEN} even though no {@code @RestControllerAdvice} exists here. The pair
 * implements both filter-chain hooks in one place:
 * <ul>
 *   <li>{@link AuthenticationEntryPoint} — no (or invalid) token → {@code 401
 *       NOTIFICATION_UNAUTHORIZED}.</li>
 *   <li>{@link AccessDeniedHandler} — valid token, insufficient role → {@code 403
 *       NOTIFICATION_FORBIDDEN}.</li>
 * </ul>
 *
 ...
 * keeps the body byte-identical to what the sibling services' {@code GlobalExceptionHandler}s
 * render.
 *
...
public class SecurityProblemSupport implements AuthenticationEntryPoint, AccessDeniedHandler {
    ...
    public void commence(HttpServletRequest request, HttpServletResponse response,
        ...
        write(response, HttpStatus.UNAUTHORIZED, NotificationErrorCode.NOTIFICATION_UNAUTHORIZED,
    ...
    public void handle(HttpServletRequest request, HttpServletResponse response,
        ...
        write(response, HttpStatus.FORBIDDEN, NotificationErrorCode.NOTIFICATION_FORBIDDEN,
                "Insufficient permissions for this resource");
    }

    /**
     * Writes the RFC 9457 body with the same field shape the sibling services produce
     * ({@code type/title/status/detail} plus the flat custom {@code code}), so callers branch on
     * one contract no matter which service rejected them.
     */
    private void write(HttpServletResponse response, HttpStatus status, NotificationErrorCode code,
