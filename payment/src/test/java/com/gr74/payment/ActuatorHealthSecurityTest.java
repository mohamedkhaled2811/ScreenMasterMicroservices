 ...
 * chain, real actuator, real webhook controller):
 * <ul>
 ...
 *   <li>{@code POST /payments} without a token is {@code 401 PAYMENT_UNAUTHORIZED} with a coded
 *       ProblemDetail body — the zero-trust proof at the service port (calling payment
 *       directly with no token fails too, not just the gateway).</li>
 * </ul>
 *
...
class ActuatorHealthSecurityTest {
    ...
    @Test
    @DisplayName("POST /payments with no token is a coded 401 (zero trust at the service port)")
