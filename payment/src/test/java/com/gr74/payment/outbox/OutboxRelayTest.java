...
class OutboxRelayTest {
    ...
    @Test
    @DisplayName("publish-then-mark, in claim order; the wire eventId is the outbox row id")
    void publishesThenMarksInOrder() {
        OutboxMessage first = message(7L, OutboxEventType.PAYMENT_SUCCEEDED,
                RabbitConfig.PAYMENT_SUCCEEDED_ROUTING_KEY,
                "{\"paymentId\":500,\"bookingId\":1001}");
        OutboxMessage second = message(8L, OutboxEventType.PAYMENT_FAILED,
                RabbitConfig.PAYMENT_FAILED_ROUTING_KEY,
                "{\"paymentId\":501,\"bookingId\":1002}");
    ...
    void publishedPayloadCarriesTheOutboxRowId() {
        OutboxMessage row = message(42L, OutboxEventType.PAYMENT_SUCCEEDED,
    ...
    void brokerDownLeavesRowsPending() {
        OutboxMessage row = message(9L, OutboxEventType.PAYMENT_SUCCEEDED,
                RabbitConfig.PAYMENT_SUCCEEDED_ROUTING_KEY, "{\"paymentId\":500}");
